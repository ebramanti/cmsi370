...
\begin{document}
\maketitle
\abstract{A proposed interface design for how to bring real-time strategy to the mobile gaming scene. % JD: Not a sentence!
I will argue that the biggest challenge to real-time strategy on mobile is a combination of overwrought interface design and poor controls. With a simplified and clean interface combined with intuitive, easy-to-remember controls, it will provide the best possible mobile RTS experience for users.}
\pagebreak
...
% JD: The information in your introduction sounds good, but strictly speaking
%     they are not all common knowledge and ideally should have cited sources.
...
% JD: This is a decent idea, but how about the use of multiple fingers too?
%     That might be nice for unit selection.
	\subsection{Minimap}
...
% JD: Again, nice understandable ideas, but I think you are missing some opportunities.
%     First, many mobile devices now have very high DPI, and a minimap sounds like a
%     prime candidate for leveraging that.  Second, why just one corner?  For convenience,
%     why not allow the minimap to launch from more than one spot?  Third, "tap" isn't
%     the only thing you can do on a touch screen.  Have you considered "swipes" or
%     "pinches?"  Not that they may be automatically better, but I can't tell from the
%     paper (yet) how far you thought about the possibilities.
%
%     And also, this far in, you could have mentioned a few interaction design concepts
%     now.  Your reasoning is OK so far, but just leaves concepts implicit.
...
% JD: How about just drawing the path with your finger without intermediate taps?  Might
%     be quicker.  And how about...ooh, ooh: using multitouch to specify formations?
%     (assuming that's still a thing in modern RTS's...it was, back in the day)
	\begin{figure}[!ht]
	\begin{center}
...
% JD: So this echoes my question from earlier---you make a claim that mobile sucks
%     for RTS, but I don't see objective evidence.  Are there *no* mobile RTS's
%     out there?  Did some acknowledged authority make this statement?  Inquiring
%     minds and all that...
...
% JD: I admit I don't understand this one very well.  I need an illustration here.
\section{Design Rationale}
	\subsection{Unit Selection}
	\begin{figure}[!ht]
	\begin{center}
	\includegraphics[height = 2.5in]{starfront-selection}
	\caption{An example of unit selection in a mobile RTS, ``Starfront: Collision.'' In this example, a user does a two-finger gesture, with each finger in diagonally opposite corners, to select units.} % JD: OK, that's more like it.
	\end{center}
	\end{figure}
...
% JD: OK, the above paragraph is good.  Need moar like that one.
	\subsection{Minimap}
...
% JD: Yay something from the class!  Also, might transparency be useful here?
	\subsection{Unit Movement}
...
% JD: There's a word I *wish* you would say...but you haven't said it...
	\subsection{Building Management \& Interface}
...
% JD: What is the reasoning for associating research with the left and units with the right?
%     Is it any worse or better than research on right and units on left?
...
% JD: This is OK, but I think there's a palette of other gestures that may also help here
%     outside of just taps and drags.
...
% JD: Feels like you need a conclusion.
%
%     One more thing to consider: audio input.  What if we let the user say "Stop" to halt
%     marching troops.  Also, might a simple spoken command like "Build barracks" help in
%     *that* set of tasks?
\end{document}  