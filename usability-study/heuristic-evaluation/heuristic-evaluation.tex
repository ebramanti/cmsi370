...
% JD: Note that Fitts's Law is about *hitting* a target, not *finding* it.  Your point remains
%     valid, but it just gets associated with the incorrect principle.  This is simple visibility
%     of an option, not Fitts's Law.  Now, if you timed how long it takes for a user to click that
%     option (where the user already knows where it is), *then* you would be looking at Fitts's
%     Law as a factor.
\section{Shopzilla}
\subsection{Test 1}
...
% JD: This conclusion works as a constructive criticism of each system's current design.  Wouldn't it be great
%     to see these changes made, then to run the tests again, to see if the results show measurable improvement?
%     For the moment, we can only dream LOL.
